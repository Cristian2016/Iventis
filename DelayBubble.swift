...
public class DelayBubble: NSManagedObject {
...
extension DelayBubble {
    ...
    class Coordinator {
        ...
        //MARK: -
        weak var sdb: DelayBubble?
        
        private var initialClock:Float { sdb?.initialDelay ?? 0 }
        ...
        func update(_ moment:Moment) { //main Thread
            ...
            guard
                ...
                let currentClock = sdb?.currentDelay
            else { return }
            ...
            switch moment {
                ...
                case .user(let action) :
                    switch action {
                        ...
                        case .pause:
                            cancellable = []
                            valueToDisplay = sdb?.currentDelay ?? -666
                            animate = false
        ...
        private func startBubble(_ elapsedSinceFirstStart: Float?) {
            ...
            DispatchQueue.main.async {
                if let sdb = self.sdb {
                    ...
                    NotificationCenter.default.post(name: .killDelayBubble, object: nil, userInfo: info)
                    
                    self.cancellable = []
                    sdb.currentDelay = 0 //only to set sdb.state to .finished
                }
            }
        }
        
        // MARK: - Init Deinit
        init(_ sdb:DelayBubble) {
            self.sdb = sdb
            valueToDisplay = sdb.currentDelay
    ...
    var state: State {
        if pairs_.isEmpty { return .brandNew }
            ...
            if currentDelay == 0 { return .finished }
            let lastPair = pairs_.last
            return lastPair?.pause != nil ? .paused : .running
        }
    }
    
    var pairs_:[DelayBubblePair] {
        get { pairs?.array as? [DelayBubblePair] ?? [] }
