...
struct BubbleStickyNote: View {
    ...
    var body: some View {
        HStack (spacing:0) {
            Rectangle()
                .fill(hasCalendar ? Color.calendar : .clear)
                .frame(width: hasCalendar ? 10 : 0, height: stickyHeight)
            Text(content)
                .padding(textPadding)
        }
...
struct BubbleStickyNote_Previews: PreviewProvider {
    static var previews: some View {
        BubbleStickyNote(content: "Nice", hasCalendar: .constant(true))
    }
}
