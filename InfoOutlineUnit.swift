...
extension InfoOutlineUnit {
    struct Input:Identifiable {
        ...
        static let aepPair = Input(title: "A pair (start-pause pair) is a subunit of an entry. In other words each entry contains at least one pair. Each time the user starts and pauses a bubble, a new entry will be created", image: "pair", footnote: "If an entry corresponds to a calendar event, the entrie's pairs are subevents")
    }
}
