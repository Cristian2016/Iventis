...
extension InfoOutlineUnit {
    struct Input:Identifiable {
        ...
        static let aepEntry = Input(title: "An entry groups pairs together. An entry (group of pairs) is similar to a calendar event. 'Calendar-enabled' bubbles have entries which correspond to calendar events. To separate groups of pairs from each other, the entry must be 'finished'. To finish an entry the user *\(Image.longPress) long-presses* on seconds", image: "entry")
