...
extension InfoOutlineUnit {
    struct Input:Identifiable {
        ...
        static let aepPair = Input(title: "The pair (start-pause pair) is the simplest duration component. It has start and pause dates, duration and optionally a user note. Each time the user taps seconds either a start or a pause date is created. It means that two taps create a pair", image: "pair", footnote: "If an entry corresponds to a calendar event, the pairs of an entry are similar to the subevents of an event")
