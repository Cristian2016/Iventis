...
extension InfoOutlineUnit {
    struct Input:Identifiable {
        ...
        static let aepEntry = Input(title: "An entry groups pairs together. An entry (group of pairs) is similar to a calendar event. 'Calendar-enabled' bubbles have entries which correspond to calendar events. To close an entry the user must  *\(Image.longPress) Long Press* on seconds", image: "entry")
